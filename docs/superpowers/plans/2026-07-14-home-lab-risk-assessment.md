...
- **Autentifikácia control-plane (overené):** k8s API `6443` anonymný read secrets → `401 Unauthorized`; kubelet `10250` anonymne → `Unauthorized`; etcd `2379` bez klientskeho certifikátu → bez odpovede (mTLS). Komponenty teda sú sieťovo dosiahnuteľné, ale vynucujú autentifikáciu `[overené]`.
- **NFS:** `/data` exportované na `10.172.16.0/24` (VPN) **a** `172.16.1.0/24` (LAN) s `rw, sync, no_subtree_check, root_squash, all_squash (anonuid/anongid=1000), sec=sys, insecure` `[overené: exportfs -v]` → ktokoľvek z LAN/VPN môže mountnúť a čítať/zapisovať ako uid 1000 (bez root/priv-esc).
...
- **S2-02 — Vysoká — Zbytočne vystavený k8s control-plane (autentifikovaný).** Cesta: `6443` API, `10250` kubelet, **`2379` etcd** dosiahnuteľné z VPN `[overené]`; **každý komponent vynucuje autentifikáciu** (API 401, kubelet Unauthorized, etcd mTLS) `[overené]`. Dopad: nie je to okamžité prevzatie, ale zbytočný útočný povrch — CVE v control-plane, brute/credential útoky, a bezpečnosť etcd stojí na ochrane klientskych certifikátov; kompromitácia ktoréhokoľvek = plné prevzatie clustra. Odporúčanie: host firewall — etcd/kubelet viazať len na loopback/interné rozhranie; k8s API obmedziť na potrebné zdroje.
...
- **S2-05 — Vysoká — NFS `/data` exportované r/w na VPN subnet.** Cesta: `mount -t nfs 172.16.1.11:/data` z ktoréhokoľvek peera `[overené: exportfs -v — rw, insecure na `10.172.16.0/24`]`. Dopad: čítanie **a zápis** dát ako uid 1000; `root_squash`+`all_squash` zabraňujú root/priv-esc, ale dáta sú plne čitateľné a modifikovateľné (integrita/dôvernosť). Odporúčanie: zúžiť export (odstrániť `10.172.16.0/24` alebo len konkrétne hosty), `ro` kde stačí, zvážiť `sec=krb5`.
...
~1 page: the headline contrast (**bez VPN = 2 TCP porty na 1 host; s VPN = plný L3 na celý lab, žiadne ACL**); the top risks per scenario (S1-01/02/03 for scenario 1; S2-01 and S2-03 Kritická, S2-02/04/05/08 Vysoká for scenario 2); the current-vs-latent framing (dnes len `cwwk`+MAAS/Omada; celý cloud latentne); and the P1 actions. Reference finding IDs.
