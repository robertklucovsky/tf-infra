...
Toto hodnotenie porovnáva dva prístupové scenáre do domáceho labu cez perimeter ER605: prístup z internetu bez WireGuardu (kapitola 3) a prístup autorizovaného WireGuard klienta vrátane sub-prípadu kompromitovaného klienta (kapitola 4). Naprieč oboma scenármi platí jeden kľúčový kontrast: **bez WireGuardu vidí útočník z internetu iba 2 TCP porty (80 a 443), forwardované na jediný hostiteľ** `172.16.1.11` cez in-cluster Cilium gateway `[overené]`; **akýkoľvek autorizovaný WireGuard klient naopak získava plnú L3 konektivitu na celý lab, na všetkých portoch, bez akéhokoľvek ACL** — pretože gateway ACL na routri je vypnuté a všetky tri VLAN majú `isolation=false` `[overené]`. Tento rozdiel v povrchu útoku (2 porty na 1 host vs. plný L3 na celú sieť bez filtrovania) je najdôležitejším zistením celej správy.

**Scenár 1 — bez WireGuardu.** Z internetu je dosiahnuteľný výhradne port-forwarding TCP 80/443 na `cwwk` a UDP 51820 WireGuard listener priamo na routri; žiadna databáza, SSH, NodePort ani etcd nie je z internetu dosiahnuteľný `[overené]`. Aj v rámci tohto zúženého povrchu vznikajú tri zistenia so závažnosťou Vysoká: neautentifikované vystavené UI Prometheus, Alertmanager a Mailpit (**S1-01**), standalone prihlasovacie stránky ArgoCD, pgAdmin, Grafana, Nexus a Zot vystavené brute-force/cred-stuffing bez WAF, MFA či rate-limitu (**S1-02**, Vysoká pri pgAdmin a ArgoCD, Stredná pri ostatných) a Keycloak bežiaci v neprodukčnom móde `start-dev` na verejne dostupnom `auth.klucovsky.com` (**S1-03**).

**Scenár 2 — autorizovaný WireGuard klient (+ kompromitovaný klient).** Po pripojení do tunela dosiahne akýkoľvek peer — legitímny alebo kompromitovaný (ukradnuté zariadenie, unesený kľúč) — celú podsieť `172.16.1.0/24` aj routovateľné, momentálne prázdne `172.16.2.0/24` a `172.16.3.0/24`, na všetkých portoch vrátane NFS `/data` `[overené]`. Dve zistenia dosahujú závažnosť Kritická: úplná absencia segmentácie medzi VPN klientom a infraštruktúrou (**S2-01**) a dosiahnuteľnosť kombinovaného MAAS/Omada boxu z VPN, ktorý ovláda provisioning fyzických serverov aj sieťovú kontrolnú rovinu (**S2-03**). K nim sa pridávajú štyri zistenia so závažnosťou Vysoká: zbytočne vystavený, hoci autentifikovaný, k8s control-plane vrátane etcd (**S2-02**), PostgreSQL v plaintexte na NodePort `30432` (`sslmode=disable`) obsahujúci Terraform state `[predpoklad]` (**S2-04**), NFS export `/data` na čítanie aj zápis do VPN podsiete `10.172.16.0/24` (**S2-05**) a dosiahnuteľné manažmentové roviny switchov a Omada kontroléra (**S2-08**).

**Súčasný stav vs. latentné riziko.** K dátumu hodnotenia je živých len málo komponentov: uzol `cwwk`, kombinovaný MAAS/Omada box a Omadou spravované switche `[overené]`. Celý súkromný cloud OpenStack/Ceph/Juju (9 serverov) je vypnutý `[overené]`. Absencia segmentácie a ACL na plochej sieti však platí rovnako pre vypnutý aj zapnutý stav — po opätovnom zapnutí cloudu sa ten istý nedostatok premietne do podstatne väčšieho blast radius vrátane OpenStack API, Ceph a BMC/IPMI. Súčasné živé komponenty preto treba chápať ako spodnú hranicu rizika, nie ako jeho strop.

**Okamžité kroky (P1).** Kapitola 6 uvádza štyri opatrenia s najvyššou prioritou: (1) gateway ACL na WireGuard tuneli (`10.172.16.0/24` → infra, default-deny) a zapnutie inter-VLAN isolation; (2) host firewall na `cwwk`, ktorý viaže etcd a kubelet len na loopback/interné rozhranie a obmedzuje NodePorty na potrebné zdroje; (3) presun neautentifikovaných UI (Prometheus, Alertmanager, Mailpit) za forward-auth alebo len na VPN; (4) zúženie NFS exportu `/data` odstránením VPN podsiete z exportu alebo obmedzením na konkrétne hosty. Tieto štyri kroky spoločne uzatvárajú väčšinu kriticky a vysoko hodnotených zistení oboch scenárov.
...
**S1-03 — Vysoká — Keycloak beží v `start-dev` `[predpoklad: z repo konfigurácie]` a je vystavený na internete.** Popis: `auth.klucovsky.com` beží v dev móde. Útočná cesta: priamy prístup na verejne dostupný IdP bežiaci v neprodukčnej konfigurácii. Dopad: dev mód vypína produkčné hardeningové mechanizmy (povolené HTTP, voľnejšie nastavenia hostname/cache), čo je nevhodné pre exponovaný identity provider, ktorý chráni prístup k RustFS. Závažnosť: **Vysoká**. Odporúčanie: prejsť na produkčný mód (`build`+`start`, `KC_HOSTNAME` v strict móde, len HTTPS) a skryť administrátorskú konzolu.

**S1-04 — Stredná — CVE povrch vystavených verzií (vrátane beta).** Popis: verejne dostupné sú aj verzie softvéru, ktoré nemusia byť produkčne vyzreté, napríklad RustFS `1.0.0-beta.8`, Keycloak `26.6.4` `[predpoklad: z repo konfigurácie, bežiaca verzia neoverená]`, Nexus, Zot a Grafana. Útočná cesta: exploatácia známej alebo neznámej zraniteľnosti vo vystavenej verzii. Dopad: beta alebo nepatchnutý softvér vystavený na internete predstavuje riziko zneužitia známych zraniteľností. Závažnosť: **Stredná**. Odporúčanie: zaviesť pravidelný patch cadence, sledovať CVE pre používané verzie a nevystavovať beta služby verejne. Konkrétne CVE identifikátory nie sú súčasťou tohto hodnotenia — `[predpoklad]`, doplniť pri revízii jednotlivých verzií.

**S1-05 — Nízka — TCP 80 otvorené (redirect na 443) + chýbajúce HSTS.** Popis: port 80 je otvorený a presmerováva na 443, no chýba HSTS hlavička. Útočná cesta: chýbajúca HSTS hlavička vytvára priestor pre SSL-strip útok pri prvom kontakte klienta. Dopad: minimálny. Závažnosť: **Nízka**. Odporúčanie: nasadiť HSTS s preload, prípadne vystaviť len port 443.

**S1-06 — Nízka/Info — UDP 51820 WireGuard na WAN.** Popis: WireGuard listener počúva na UDP 51820. Útočná cesta: listener je dosiahnuteľný priamo na WAN rozhraní routra. Dopad: minimálny/akceptovateľný — toto je *správna* kontrola: WireGuard je voči neautentifikovaným sondám tichý (nereaguje) a používa silnú kryptografiu. Závažnosť: **Nízka/Info**. Odporúčanie: udržiavať firmware ER605 patchnutý; prípadne zvážiť zmenu portu ako security-by-obscurity opatrenie s nízkou hodnotou.
...
**S2-06 — Stredná — Ďalšie NodePorty: RustFS `30910/30911`, Tempo `30417/30418`.** Popis: RustFS (`30910/30911`) a Tempo (`30417/30418`) bežia na ďalších NodePortoch. Útočná cesta: tieto NodePorty sú dosiahnuteľné z VPN. Dopad: prístup k object-store admin rozhraniu/API a k tracing dátam. Závažnosť: **Stredná**. Odporúčanie: obmedziť zdrojové adresy firewallom; vynútiť autentifikáciu na RustFS admin rozhraní.

**S2-07 — Stredná/Nízka — Info-disclosure služby: node_exporter `9100`, Hubble `4244`, rpcbind `111`.** Popis: node_exporter (`9100`), Hubble (`4244`) a rpcbind (`111`) bežia na node. Útočná cesta: tieto porty vystavujú interné informácie bez ďalšej ochrany. Dopad: únik interných metrík, topológie a network flow dát, možnosť enumerácie cez RPC. Závažnosť: **Stredná/Nízka**. Odporúčanie: viazať tieto služby na interné rozhranie alebo ich obmedziť firewallom.

**S2-08 — Vysoká — Management roviny switchov a Omada dosiahnuteľné z VPN.** Popis: manažment rozhrania switchov (`172.16.1.23/24/25`) aj Omada kontrolér (`8043`) bežia v sieti. Útočná cesta: sú dosiahnuteľné z tunela. Dopad: prekonfigurácia siete, VLAN alebo port mirroringu môže viesť k prevzatiu celej siete. Závažnosť: **Vysoká**. Odporúčanie: oddeliť mgmt VLAN od VPN; nasadiť ACL s politikou default-deny na mgmt rozhrania.
...
**C-04 — PostgreSQL v plaintexte.** Databáza bežiaca bez TLS (zistenie S2-04) vystavuje poverenia aj Terraform state `[predpoklad]` riziku odpočúvania na trase.
...
**C-06 — Keycloak v dev režime.** Identity provider chrániaci prístup k RustFS beží v móde `start-dev` `[predpoklad: z repo konfigurácie]`, ktorý vypína produkčné hardeningové mechanizmy (zistenie S1-03).

**C-07 — Secrets vo vystavenom etcd + TF state v dosiahnuteľnom Postgrese.** Citlivé dáta — secrets uložené v etcd a Terraform state (vrátane superuser poverení) `[predpoklad]` v Postgrese — sú uložené v komponentoch, ktoré sú sieťovo dosiahnuteľné z VPN (zistenia S2-02 a S2-04), čo zvyšuje hodnotu prípadného kompromisu.
...
- **Admin aplikácie (ArgoCD, pgAdmin, Nexus, Grafana, Zot):** presunúť za SSO forward-auth alebo len na VPN; vynútiť MFA. → S1-02.
...
| Správa secrets (etcd/TF state `[predpoklad]`) | Secrets management | **Fail** | C-07 |
