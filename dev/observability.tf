...
resource "kubectl_manifest" "route_grafana" {
  ...
  depends_on = [kubectl_manifest.gateway, helm_release.prometheus_stack]
}
...
resource "kubectl_manifest" "route_prometheus" {
  yaml_body = <<-YAML
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
resource "kubectl_manifest" "route_alertmanager" {
  yaml_body = <<-YAML
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
